...
class BrowseChannelsController extends Controller
{
    ...
    public function browse(Category $category = null, String $slug = null)
    {
        ...
        // if (!$category) {
        //     $channels = User::isStreamer()->with(['categories', 'latestVideo']);
        // } else {
        //     $channels = $category->users()->isStreamer()->with(['categories', 'latestVideo']);
        // }

        if (!$category) {
            $channels = User::isStreamer()
            ->with(['categories', 'latestVideo'])
            ->when($request->filled('skin_tone'), function ($query) use ($request) {
                $query->where('skin_tone', $request->skin_tone);
            })
            ->when($request->filled('status'), function ($query) use ($request) {
                $query->where('live_status', $request->status);
            });
        } else {
            $channels = User::isStreamer()
                ->with(['categories', 'latestVideo'])
                ->when($request->filled('skin_tone'), function ($query) use ($request) {
                    $query->where('skin_tone', $request->status);
                })
                ->when($request->filled('status'), function ($query) use ($request) {
                    $query->where('live_status', $request->status);
                });
