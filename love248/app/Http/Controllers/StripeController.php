...
class StripeController extends Controller
{
    ...
    // purchase
    public function purchaseToken($tokenPack, Request $request)
    {
    ...
    // get client secret
    public function paymentIntent($tokenPack, $saleId)
    {
        $stripe = new StripeClient(opt('STRIPE_SECRET_KEY'));

        $stripeAmount = $tokenPack * 100;
        ...
        $pi = $stripe->paymentIntents->create(
            [
                ...
                'metadata' => ['sale_id' => $saleId, 'token' => $tokenPack]
            ]
        );
