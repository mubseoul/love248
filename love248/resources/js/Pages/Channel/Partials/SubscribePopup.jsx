  ...
  return (
    <>
      ...
      <div className="d-block d-md-none">
        {!userIsSubscribed ? (
          <button
            className="mobile-action-btn btn btn-outline-warning d-flex flex-column align-items-center justify-content-center text-center"
            onClick={(e) => showSubscribe(e)}
          >
            <FaGrinStars size={22} className="mb-1" />
            <span className="small fw-bold">{__("Subscribe")}</span>
          </button>
        ) : (
          <button
            className="mobile-action-btn btn btn-warning d-flex flex-column align-items-center justify-content-center text-center"
            onClick={(e) => Inertia.visit(route("mySubscriptions"))}
          >
            <FaGrinStars size={22} className="mb-1 text-dark" />
            <span className="small fw-bold text-dark">{__("Subscribed")}</span>
          </button>
        )}
      </div>
      ...
      <div className="d-none d-md-block">
        {!userIsSubscribed ? (
          <button
            ...
            onClick={(e) => showSubscribe(e)}
          >
            <FaGrinStars className="me-2" />
            <span className="button-text">{__("Subscribe")}</span>
          </button>
        ) : (
          <button
            className="me-2 btn text-uppercase position-relative d-flex align-items-center justify-content-center"
          ...
          >
            <FaGrinStars className="me-2" />
            <span className="button-text">{__("Subscriptions")}</span>
          </button>
        )}
      </div>
          ...
          <div className="py-3">
            ...
            {tiers.map((tier) => (
              <div key={`tier-${tier.id}`} className="mb-4">
                <h3 className="text-indigo-800 text-lg">{tier.tier_name}</h3>
