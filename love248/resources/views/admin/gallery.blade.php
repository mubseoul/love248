...
<strong>{{ __('message.gallery_title') }}</strong>
@endsection
...
<div class="alert alert-info d-flex align-items-center" role="alert">
    ...
    <div>{{ __('message.gallery_alert') }}</div>
</div>
    ...
    <div class="col-sm-12">
        <div class="card">
                    ...
                    <table id="commentTable" class="data-tables table custom-table movie_table" data-toggle="data-table">
                        ...
                        @foreach($gallery as $g)
                            <tr>
                                <td>
                                    <div class="d-flex">
                                        <img src="{{$g->thumbnail}}" alt="image" class="rounded-2 avatar avatar-55"  />
                                        <div class="d-flex flex-column ms-3 justify-content-center">
                                            <h6 class="text-capitalize">{{$g->title}}</h6>
                                        </div>
                                    </div>
                                </td>
                                <td>
                                    <a target="_blank" href='{{route("channel", ["user"=> $g->streamer->username]) }}' class="text-primary fw-bold"> {{ '@' . $g->streamer->username}} </a>
                                </td>
                                <td>{{ $g->price < 1 ? __("Free") : $g->price . __(" tokens") }}</td>
                                <td>
                                {{ $g->category->category }}
                                    <!-- <a target="_blank" href="{{ route('videos.browse', ['videocategory' => $g->category->id, 'slug' => '-' . $g->category->slug]) }}" class="text-info"> <i class="fa-solid fa-tags"></i> {{ $g->category->category }} </a> -->
                                </td>
                        ...
                        <td>
                            @if($g->status == 1)
                                ...
                                <br />
                                <a href="/admin/gallery-approve?id={{ $g->id }}" class="text-xs text-primary fw-bold">{{ __("Mark as Verified") }}</a>
                                @endif
                        </td>
                            <td>
                                <div class="d-flex align-items-center list-user-action">
                                    @can('gallery-delete')
                                    <a href="/admin/galleries?remove={{ $g->id }}"
            ...
            <div class="alert alert-warning d-flex align-items-center" role="alert">
                ...
                <div>
                {{ __('No gallery uploaded by streamers.') }}
            </div>
            @endif
            </div>
        </div>
