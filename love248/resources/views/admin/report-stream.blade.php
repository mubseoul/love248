...
@section('section_title')
    <strong class="text-capitalize">{{ __('message.report_stream')  }}</strong>
@endsection
                            ...
                            <table class="table border-collapse w-full bg-white text-stone-600 dataTable">
                                <thead>
                                    <tr>
                                        ...
                                        <x-th>{{ __('message.reported_item')  }}</x-th>
                                ...
                                <tbody>
                                    @foreach ($contents as $c)
                                        <tr>
                                            ...
                                            <x-td>
                                                <x-slot name="field">{{ __('Item')  }}</x-slot>
                                                {{ $c->item }}
                                            </x-td>
                                            <x-td>
                                                <x-slot name="field">{{ __('Item')  }}</x-slot>
                                                <div class="iq-button">
                                                    <a target="_blank" href={{ $c->item_id }} class="btn btn-sm btn-primary">View Stream</a>
                                                </div>
                                            </x-td>
                                        </tr>
                                    @endforeach
